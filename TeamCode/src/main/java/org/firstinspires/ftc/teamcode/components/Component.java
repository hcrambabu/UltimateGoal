...
public abstract class Component {
    ...
    public static double SERVO_POS_ERROR_BOUNDARY = 0.03;
    ...
    public boolean isOpmodeActive() {
        return robot.opmode.opModeIsActive() && !robot.opmode.isStopRequested();
    }

    public void waitForTime(double sec) {
        double expTime = robot.opmode.getRuntime() + sec;
        try {
            while (isOpmodeActive() && robot.opmode.getRuntime() < expTime) {
                //robot.opmode.idle();
                Thread.sleep(50);
            }
        } catch (Exception ex){}
